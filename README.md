...
After injecting and waiting the specific time points for all the individuals the blood samples are prepared. The ethanol concentration was measured in an spectrophotometer, so the raw data we are going to get is absorvance values.

##### Blanks & ethanol solution sample
- The wet lab scientist made blank samples, to enable measurement of the actual ethanol concentration in the injection solution they dilute the stock ethanol solution 100 times, otherwise they treated the blank samples the same way as the samples containing blood.
##### Sample treatment
All samples (including the blanks) are centrifuged at 2 500 rpm in the centrifuge (1.5 ml tubes) for 5 minutes. Transfer 0.9 ml supernatant to new tubes.
...
The scientist mixed the sample and the different master mix (not relevant for the data analysis) and measured the absorbency of the samples against blank, at 340 nm.
